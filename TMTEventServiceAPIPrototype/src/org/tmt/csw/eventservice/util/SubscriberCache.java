...
public class SubscriberCache {
	 ...
	 * @param listener
	 */
	public void addCallbackToCache(String topic, String consumerQueue,
	 ...
	 * @param topic
	 */
	public void removeListenerFromCache(String topic) {
		if (subscriberCacheMap.containsKey(topic)) {
	 ...
	 * @param callback
	 */
	public void removeCallbackFromCache(String topic, String consumerQueue,
			String callback) {
		if (subscriberCacheMap.containsKey(topic)) {
			List<QueueCallBackMapping> listofQueues = subscriberCacheMap
					.get(topic);
	 ...
	 * @param callback
	 */
	public void removeAllCallbacksFromCache(String topic, String consumerQueue) {
		if (subscriberCacheMap.containsKey(topic)) {
			List<QueueCallBackMapping> listofQueues = subscriberCacheMap
					.get(topic);
			Object[] queues = listofQueues.toArray();
	 ...
	 */
	public TopicListener getCallbackListener(String topic,
	 ...
	 */
	public Collection<TopicListener> getAllCallbackListeners(String topic,
	...
	/**
	 * Checks any pending subscribers for a given topic
	 * 
	 * @param topic
	 * @param consumerQueue
	 * @return boolean
	 */
	public boolean pendingSubscribersForTopic(String topic, String consumerQueue) {
		if (subscriberCacheMap.containsKey(topic)
				&& subscriberCacheMap.get(topic).size() != 0) {
			List<QueueCallBackMapping> listofQueues = subscriberCacheMap
					.get(topic);
			...
			for (Iterator<QueueCallBackMapping> iterator = listofQueues
				...
				if (queueCallBackMapping.getQueueName().equals(consumerQueue)) {
					if (queueCallBackMapping.getMap().size() == 1) {
						return false;
					} else {
						return true;
					}
