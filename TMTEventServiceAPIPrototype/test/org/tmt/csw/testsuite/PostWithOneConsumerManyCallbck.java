...
/**
 * Post Message with one consumer and multiple callbacks--> Only one
...
public class PostWithOneConsumerManyCallbck extends TestCase {
	...
	public void testPostWithOneConsumerManyCallbck()
		...
		// consumerOne Subscribe to a Topic with two call backs -mockEventCallbackImplOne,mockEventCallbackImplTwo
		EventConsumerOne consumerOne = new EventConsumerOne();
		System.out.println("consumer Subscribes to a Topic with two call backs");
		...
		if (evt != null || secondEvt != null) {
			if (evt != null) {
				...
				System.out
						.println("Event received by Consumer callback-1 = ["
								+ evt + "]");
			...
			if (secondEvt != null) {
				...
				assertNotNull(
						...
						secondEvt);
				System.out
						.println("Event received by Consumer callback-2 = ["
								+ secondEvt + "]");
