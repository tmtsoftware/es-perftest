...
/**
 * This class generates the HTML report from the statistics collected in the database. It is also responsible for
 * creating the required graphs by invoking necessary classes.
 * 
 * 
 */
public class ReportGenerator {
   ...
   public static void main(String[] args) throws SQLException, IOException, ClassNotFoundException {
      ...
      if (args.length == 0) {
         ...
         System.out
                  .println("This utility requires arguments in the format <report-type>:<data-token>, you can provide multiple combinations of <report-type>:<data-token> separated by commas");
         System.out
                  .println("Run this utility again with the switch -showtypes to see all the supported report types\n");
         return;
      } else if (args.length == 1 && "-showtypes".equalsIgnoreCase(args[0])) {
         String reportQueriesFile = "properties/queries.properties";
         Map<String, String> commonSettingsMap = ReportUtils.loadConfig("common-settings.properties");
         ...
         Map<String, String> sqlQueryMap = ReportUtils.loadConfig(reportQueriesFile);
   ...
   public void process(String strGraphTypesLst) throws Exception {
      ...
      GraphGeneratorFactory graphFactory = GraphGeneratorFactory.getInstance();
      String[] arrGraphTypes = ReportUtils.strSplit(strGraphTypesLst, BCSuiteConstants.COMMA);
      for (int i = 0; i < arrGraphTypes.length; i++) {
         ...
         String[] arrGTypeToken = ReportUtils.strSplit(strGraphTypeToke, BCSuiteConstants.COLON);
         if (arrGTypeToken.length < 2) {
            System.out.println("The part argument [" + strGraphTypeToke
                     + "] is not correct. You might have missed to provide the token or the report type.");
            continue;
         }
    ...
    * @throws IOException
    */
   public void generateHTMLReport(List<Graph> graphs, String reportFileName) throws IOException {
      BufferedWriter bw;
      bw = new BufferedWriter(
               new FileWriter(ReportUtils.getFile(BCSuiteConstants.FOLDER_PATH_BCSUITE + reportFileName)));
