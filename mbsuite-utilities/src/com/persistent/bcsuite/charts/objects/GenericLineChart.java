...
public class GenericLineChart implements Graph {
   private static final Logger logger = Logger.getLogger(GenericLineChart.class);
   ...
   private XYSeriesCollection createDataset() throws IOException, ClassNotFoundException, SQLException {
      ...
      try {
         if (isPercentileRequired(reportType)) {
            double[] distribution = ReportUtils.getAxisValueSet(BCSuiteConstants.YAXIS, tableVal);
            double[][] ltncyDistriValues = ReportUtils.calculatePercentile(distribution);
            tableVal = ltncyDistriValues;
            xLabel="Percentile";
         }
      } catch (Exception e) {
         logger.error("Exception while calculating percentile, will not plot percentile graph");
      }
      ...
      for (int i = 0; i < tableVal.length; i++) {
         int j = 0;
         XYDataItem dataItem = new XYDataItem(tableVal[i][j], tableVal[i][j + 1]);
   ...
   private boolean isPercentileRequired(String reportType2) throws IOException {
      Map<String, String> commonSettingsMap = ReportUtils.loadConfig("common-settings.properties");
      if (commonSettingsMap != null && commonSettingsMap.size() > 0) {
         String percentileReportTypes = commonSettingsMap.get("percentileReportTypes");
         logger.debug("Percentile Report Types loaded from config [" + percentileReportTypes + "]");
         if (percentileReportTypes != null && percentileReportTypes.trim().length() > 0) {
            String[] reportTypesList = percentileReportTypes.split(",");
            if (reportTypesList != null && reportTypesList.length > 0) {
               for (String s : reportTypesList) {
                  if (s.equalsIgnoreCase(reportType2))
                     return true;
               }
            }
         }
   ...
   public String generateTable() throws IOException {
      ...
      strBud.append(BCSuiteConstants.TD_OPEN);
      strBud.append("<img src=\"" + token + BCSuiteConstants.FOLDER_PATH_IMAGE + this.reportType + ".png"
               + "\" alt=\"graph\"> ");
      strBud.append(BCSuiteConstants.TD_CLOSE);
   ...
   public File getFile() throws IOException {
      fileName = BCSuiteConstants.FOLDER_PATH_BCSUITE + token + BCSuiteConstants.FOLDER_PATH_IMAGE + this.reportType
               + ".png";
