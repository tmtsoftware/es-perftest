...
if [[ $1 = "" || $2 = "" || $3 = "" || $4 = "" || $5 = "" || $6 = "" ]]; then
echo "No parameter supplied :  valid sequence is <./run-publisher-vary-size.sh> <MIN-sub> <increment-By> <MAX-sub> <key> <groupName> <token>"
else
...
echo "Publisher for varying-sub started..."
...
#loads the external jars from "ext-lib" , utility jars from "lib" & configuration parameter from .tmpl file inside  "config" dir 
  java -d64 -XX:-UseGCOverheadLimit  -Xms4048M -Xmx14048M -cp  "$OSPL_HOME/jar/dcpssaj.jar:../../ext-lib/*":"../../ext-lib/rti/*":"../../export/data-files/*":"../../lib/*":"../../config/":"../../export/"   com.persistent.bcsuite.process.Generator $key $groupName $token $iteration  >> logs/run-publisher-vary-sub.log  &
