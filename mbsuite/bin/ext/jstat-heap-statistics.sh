...
function usage() {
    ...
    echo " -d <duration> for how many times test will be run (in sec)"
    echo " -i <increment> increment of test (in sec)"
...
while getopts hvp:s:j:w:c:d:i: opt ; do
case ${opt} in
...
echo "time,pid,heapcurrent(kb),heapmax(kb),heapratio(%)"


for (( j=1; j <= $duration; j=j+$increment )) 
  do
	
echo "$(date +"%s"),$pid,$heap,$heapmx,$heapratio"
