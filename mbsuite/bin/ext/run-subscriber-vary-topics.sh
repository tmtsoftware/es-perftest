...
if [[ $1 = "" || $2 = "" || $3 = "" || $4 = "" || $5 = "" || $6 = "" ]]; then
echo "No parameter supplied :  valid sequence is <./run-subscriber-vary-topics.sh> <MIN-Bytes> <increment-By> <MAX-Bytes> <key> <groupName> <token>"
else
...
for (( j=$MIN; j <= $MAX; j=j+$increment )) 
 ...
 sed -i 's/@maxTopics/'$j'/g' ../../config/subscriber-config.xml 
...
#loads the external jars from "ext-lib" , utility jars from "lib" & configuration parameter from .tmpl file inside  "config" dir 
   java -d64 -XX:-UseGCOverheadLimit -Xms4048M -Xmx14048M -cp  "$OSPL_HOME/jar/dcpssaj.jar:../../ext-lib/*":"../../ext-lib/rti/*":"../../export/data-files/":"../../lib/mbsuite-addons.jar":"../../config/":"../../lib/mbsuite-subscriber-base.jar":"../../export/"    com.persistent.bcsuite.process.Extractor $key $groupName $token $iteration  >> logs/run-subscriber-vary-topics.log &
