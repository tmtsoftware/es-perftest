...
/**
 * Publisher class that publishes messages to the subscriber.
 * 
 */
public class ThroughputPublisher extends PublisherBase {
	...
	public void init(Map<String, String> attributes) {
		...
		if (large_data || isAsynch) {
			...
			configure_flow_controller_property(flow_controller_property,
					participant, large_data);
			/* Create flowcontroller */
			flow_controller = participant
					.create_flowcontroller("throughput test flow_controller",
		...
		int samples_per_trigger = RTIQosHelper.configure_data_writer_qos(
				...
				publisher, useMulticast, isAsynch, isReliable, counter);

		data_writer = (ThroughputDataWriter) publisher.create_datawriter(
				data_topic, data_writer_qos, null, StatusKind.STATUS_MASK_NONE);
		data_writer.enable();
		/*
		 * Register Topic instance with data writer. Note: because we are not
		 * using keys this will return DDS_HANDLE_NIL
		 */
	...
	/**
	 * Configures the flow controller used to send large data asynchronously
	 * 
	 * @param flow_controller_property
	 * @param participant
	 * @param large_data
	 */
	public void configure_flow_controller_property(
