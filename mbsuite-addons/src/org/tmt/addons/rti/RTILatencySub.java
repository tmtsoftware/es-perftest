...
public class RTILatencySub extends SubscriberBase {
      ...
      public void on_data_available(DataReader reader) {
         ...
         try {
            ...
            for (int i = 0; i < dataSeq.size(); ++i) {
               recdTimeNano=System.nanoTime();
               if (((SampleInfo) infoSeq.get(i)).valid_data) {
                  message = (byte[]) ((Throughput) dataSeq.get(i)).data.getPrimitiveArray();
                  String strMessage =  new String(message);
                  strMessage = strMessage.trim();
                  
                  if(messageCounter > 999)
                  {
                     messageCounter=0;
                     int idx = strMessage.indexOf(":");
                     if(idx != -1)
                     {
                        String sentTime = strMessage.substring(0,idx);
                        long sentTimeNano = Long.parseLong(sentTime);
                        
                        addToStatisticsPool(null,strMessage.length(),new Date(),(recdTimeNano-sentTimeNano));
                     }
                  }
