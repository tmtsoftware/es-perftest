...
# dos2unix
sed -i 's/\r//' build-git.sh
sed -i 's/\r//' ../config/constants.properties
sed -i 's/\r//' dir-exist-git.sh
...
echo -e "\033[1mGit Checkout:\033[0m"
echo -e "\033[1m==============================================\033[0m"
ant -buildfile build-git.xml
cd ..
echo -e "\033[1m==============================================\033[0m"
echo  -e "\033[1mFinish Git Checkout.\033[0m"
echo -e "\033[1m==============================================\033[0m"
cd ..
...
then
	...
	exec $checkout_dir/mbsuite-packager/bin/./dir-exist-git.sh $checkout_dir/trunk $test_dir	
else
	echo "$test_dir/mbsuite directory not found!"
	cp -avr $checkout_dir/trunk/mbsuite $test_dir 
	#call the dir-exist.sh which builds publisher,subscriber,addons,utilities
	exec $checkout_dir/mbsuite-packager/bin/./dir-exist-git.sh $checkout_dir/trunk $test_dir
fi
